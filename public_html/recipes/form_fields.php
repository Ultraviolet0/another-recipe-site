...
<fieldset>
  ...
  <input type="text" id="description" name="recipe[description_rcp]" value="<?php echo h($draft['recipe']['description_rcp'] ?? ''); ?>"><br>

  <label for="servings">Servings</label><br>
  <input type="number" step=".5" min="0" id="servings" name="recipe[serving_rcp]" value="<?php echo h($draft['recipe']['serving_rcp'] ?? ''); ?>"><br>
...
<fieldset>
  ...
  <?php foreach (($draft['ingredients'] ?? []) as $i => $ing) { ?>
    <div>
      ...
      <input type="number" step=".1" min="0" id="qty-ingredient-<?php echo h($i); ?>" name="ingredients[<?php echo $i; ?>][quantity_rcping]" value="<?php echo h($ing['quantity_rcping'] ?? ''); ?>">
  ...
  <?php } ?>
  <button type="submit" name="action" value="add_ingredient" formnovalidate>Add Ingredient</button>
</fieldset>
...
<fieldset>
  ...
  <?php } ?>
  <button type="submit" name="action" value="add_direction" formnovalidate>Add Step</button>
</fieldset>
...
<fieldset>
  ...
  <p>You may upload up to 6 images (JPG/PNG/WebP). Add photos after finishing ingredients and steps (file selections reset when the page reloads).</p>
</fieldset>
